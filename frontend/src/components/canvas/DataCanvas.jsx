...
import { Database, FileDown, Layers } from 'lucide-react';
...
export default function DataCanvas({
  ...
  const exportToPDF = () => {
    ...
    // Header & Confidentiality Branding
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("ALOKA — KARNATAKA STATE POLICE INTELLIGENCE REPORT", 14, 15);
  ...
  return (
    <section className="flex-1 bg-white dark:bg-slate-900 flex flex-col relative overflow-hidden transition-colors duration-300">
      {/* Canvas Top Bar */}
      <div className="h-14 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 shrink-0 transition-colors">
        ...
        {hasResults && (
          <button 
            onClick={exportToPDF} 
            className="flex items-center gap-1.5 bg-blue-900 hover:bg-blue-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-xs transition-all cursor-pointer"
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-slate-100/60 dark:bg-slate-950 transition-colors">
        {!hasResults ? (
          <div className="flex flex-col h-full items-center justify-center text-slate-400 dark:text-slate-600 gap-2">
            <Layers className="w-10 h-10 stroke-1 opacity-50" />
            <span className="text-xs font-medium">No structured database records for this prompt.</span>
          </div>
        ) : (
